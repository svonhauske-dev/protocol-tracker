...
**Pending:** device-verify bulk-add + confirm PostHog events land (US region assumed); cut a build carrying the design-audit + empty-state passes; then the Apple Health build (18) per the activation notes; then Phase 4 (RevenueCat + gating + trial paywall). Deferred audit item: a one-tip-per-session gate (the timing-conflict + take-all InlineTips can briefly co-occur on first use — both one-time/dismissible; a hard gate risks permanently suppressing one behind the other, so left as a tradeoff).
