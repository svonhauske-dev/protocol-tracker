...
*Last updated: July 6, 2026 (cont. 2) — **Check-in reimagined + Apple Health SHIPPED to a build + user-supplement dictionary audit.** **(1) Daily check-in → one-tap "how you feel."** Replaced the three abstract energy/mood/sleep sliders with a single 5-state feeling scale (`rough→great`) after Mobbin research (Lake / Me+ / Calm / Apple State-of-Mind all show sustained daily tracking is ONE expressive tap, never multiple sliders — and Sofia, a target user: "I wouldn't fill this out daily"). Backed by the existing `daily_checkins.mood` column — **NO migration**; `mood` now semantically = OVERALL FEELING. Shared primitive `mobile/components/FeelingScale.js` (even, fully-labelled tick axis; accent/plural-safe). Energy/sleep are optional depth (edited via `CheckinSheet`, which now leads with the FeelingScale axis, not bars). **(2) Check-in OFF the home** (Sofia's call — tracking is the core home action; the feeling card was demoting it to secondary). Removed the inline day-view card entirely (today + past). Feeling is now captured in a daily **MORNING MOMENT** — `mobile/components/MorningCheckin.js`: once/day on first launch when today's feeling is unlogged, time-aware greeting at identity scale, always-dismissible ("not now"/backdrop), held confirmation beat; guarded to one show/day via `localStorage checkin_moment_seen`, suppressed just-after-onboarding — plus a persistent **"check in for today"** entry in Insights › how you feel. **(3) Insights polish** — tab `interactions`→**`guidance`**; top-bar icon `Activity`→**`TrendingUp`** (Activity read as a heartbeat, worse now that we surface HRV/resting-HR); "worth moving" nudges are now **dismissable** (× persists in `dismissed_movers`; item stays in the timing reference below). **(4) APPLE HEALTH — activated + shipping on a build.** Chosen wearable path: **Oura & Whoop both sync sleep/HRV/resting-HR INTO Apple Health**, so one HealthKit integration covers Apple-Watch AND Oura/Whoop users — direct Oura/Whoop OAuth (their proprietary Readiness/Recovery scores) deferred to phase 2. `mobile/lib/health.js` **rewritten for the healthkit v14 (Nitro) API** (verified against its type defs — the old scaffold targeted a different API: `requestAuthorization({toShare,toRead})`, queries use `{filter:{date:{startDate,endDate}},limit,ascending}`, recovery via `getMostRecentQuantitySample`). Reads sleep + HRV(SDNN) + resting HR (`readHealthSnapshot`). Installed `@kingstinct/react-native-healthkit` 14.0.2 + `react-native-nitro-modules` 0.36.1 (autolinks pod on build); added `com.apple.developer.healthkit` to `ios/Origin/Origin.entitlements` + `NSHealth{Share,Update}UsageDescription` to `Info.plist`; **Sofia enabled HealthKit on the `app.origin-protocol` App ID in the Apple Developer portal** (required before the build, else signing fails). Settings `apple health` CONNECT→CONNECTED (persists opt-in `health_enabled`; row hidden until a Health build makes `isHealthSupported()` true — never in Simulator). **"apple health · last night" readout** (sleep h · HRV · resting HR) in Insights › how you feel, null-safe + opt-in gated. **Build 19 CONNECT flow VERIFIED on Sofia's device** (row appears, iOS permission sheet grants). **Build 20 building** (this session's readout + dictionary + icon/dismissable). Phase 1b — sleep auto-fill of the check-in, a recovery track in Trends, report integration — comes AFTER build 20 confirms real data flows. **(5) User-supplement dictionary audit.** Pulled all **66** distinct supplement/med names real users actually take (`SELECT … FROM supplements`) and cross-referenced against `core/lib/interactions.js`: **31 → 62 recognized.** Root fixes: `norm()` strips NFD accents (whole Spanish-speaking user class — Magnésio, Melena de León, Vitamina c, Gabapentina), and `hasKeyword` tolerates a trailing plural `s` (Enzymes/BCAAs/Trace Minerals) END-only so the `epa`/`tirzEPAtide` guard still holds (both re-verified). Brand/Spanish keywords added (`Ferrexel`→iron, so **Levothyroxine + Ferrexel now flags** the absorption conflict; NeuroMag/Sensitol/Mitopure/MicrobiotFit/…); **14 new ingredients with evidence-based timing** (UC-II collagen = empty stomach, distinct from collagen peptides; isotretinoin = with dietary fat; allopurinol = after food; licorice/methylene-blue/methyl-guard = AM; gabapentin/dutasteride/telmisartan = consistent time); new **rule levothyroxine ↔ fiber** (psyllium binds it). 4 unidentifiable brands left UNRECOGNIZED rather than invent timing (Amplifye, Nutricidin, Theracid, Urysia — asked Sofia). Everything stays inside the TIMING-only liability scope. Commits on `terminal-elevated`: `6089974` (one-tap scale + guidance rename), `088c4d3` (off-home + moment redesign), `4271a5c` (Insights entry), `29fc804` (sheet uses axis), `3873057`/`b399363`/`b892b99` (Apple Health: reads → native config → v14 rewrite), `5375984` (health readout), `4fa3227` (TrendingUp icon + dismissable), `cba9dc0` (dictionary audit). **Pending (Sofia):** install **build 20**, connect Apple Health, confirm the readout shows real numbers; identify the 4 unknown supps; confirm "Reta"=retatrutide and what "Youth Peptides" is (currently matches collagen via "peptides").***
...
**Design-audit passes (post-build-17, JS-only — ride the next build).** (1) **Type re-locked to the primitive** — Trends/Interactions/CheckinSheet express type via `Text` `size`/`tone`/`weight` props (not inline font/size/color); fixed the adherence caption (was tertiary 3.4:1/12px → secondary 7.4:1/14px, matching Hero) and the timing disclaimers (10px/tertiary → 12px/secondary). New `Callout` primitive (non-dismissible sibling of InlineTip) replaces the note block hand-rolled in EditForm + Interactions. (2) **Nav: Trends + Interactions folded into one `Insights` surface** (`screens/Insights.js`) with a TabBar (adherence · timing) — Interactions is low-frequency and its urgent case already reaches people (edit-time note + amber icon). Top bar back to 3 icons; single Activity icon ambers on same-slot conflict + opens `initialTab='timing'`. Trends/Interactions gained an `embedded` prop (skip own header). Tab treatment matches ProtocolLibrary (header hairline + inset tabs). (3) **Empty-state + guidance coverage** — new shared `EmptyState` primitive (`// eyebrow` + `$ line ▌` + preview + CTA); Insights adherence first-use (authored empty + faint 30-bar preview instead of a lonely "—"); outcomes always render the 3 tracks (empty = visible shape) + prompt line; home **offline/error** dismissible Callout w/ retry (was a silent cache fallback); one-time **timing-conflict** InlineTip explaining the amber icon; Library saved-empty copy teaches the ⋯-menu path. Empty/error/conflict states are code-verified (need fresh-account / offline / same-slot conflict to trigger).
**Build 18 shipped** to TestFlight (design-audit + empty-state passes; buildNumber 17→18 via EAS autoincrement, committed back).
**Phase 4 — monetization machinery (JS side built; charging needs RevenueCat native + ASC = Sofia's setup + a build).** Decisions (Sofia): **roadmap free/Pro split**, **$4.99/mo · $29.99/yr (no lifetime)**, **14-day trial**. Built: (A) `mobile/lib/pro.js` — guarded RevenueCat wrapper (configure/fetchProStatus/getOfferings/purchasePackage/restorePurchases; variable-require no-op until the SDK lands; `__DEV__` override via `origin.dev.pro`). (B) `mobile/context/ProContext.js` — `ProProvider` holds `isPro` (RC entitlement `pro` || dev toggle), renders the Paywall, exposes `requirePro(feature)`/`openPaywall(feature)`; mounted in `App.js` around the app. (C) `mobile/components/Paywall.js` — terminal-styled: ★ value list, annual (hero, "save 50%") + monthly plans, 14-day-trial CTA, restore/terms/privacy; localized prices from RC offerings when available else fallback. (D) **Gates** (`requirePro`/cap → paywall): Insights (whole surface), free item cap **15** (add + bulk), PDF export, protocol sharing, 2nd+ protocol, edit-time interaction note (Pro-only), Supply→locked "track refills — Pro ›" teaser. (E) **Onboarding trial paywall** — fires once after onboarding (App.js `justOnboarded` → fire-once effect in Today), skippable = continue free. (F) Settings `dev · pro` toggle (`__DEV__` only) to exercise gates. **Verified on sim:** free→paywall (Insights + item-cap, contextual header adapts), dev-pro ON→unlocks. Commits `adb760f`/`30d6f70`/`9033b8e`.
**Pending:** confirm PostHog events land (US region assumed); **Phase 4 remaining (task #18): RevenueCat project + `RC_API_KEY_IOS` + ASC subscription products (mo $4.99 / yr $29.99, 14-day trial) + `pro` entitlement + `default` offering + `npm i react-native-purchases` + native StoreKit config + a build + sandbox-test purchase/restore** (steps in `lib/pro.js` footer; see [[mobile-bare-workflow]] re: config plugins don't apply). Also deferred: gate IF/adaptive schedule modes in the schedule picker; the Apple Health build (sleep-only, Pro); a one-tip-per-session gate (timing-conflict + take-all can briefly co-occur — tradeoff). Then Phase 5 (App Store listing + soft launch).

### Session of July 6 (cont. 2) — Check-in reimagined, Apple Health activated, dictionary audit, builds 19–20
**Daily check-in → ONE-TAP feeling.** Mobbin research (Lake/Me+/Calm/Apple State-of-Mind + Sofia's own "I wouldn't fill this out daily") → replaced 3 abstract sliders (energy/mood/sleep) with a single 5-state `rough→great` scale. `FeelingScale.js` = an even, fully-labelled tick axis (the chosen state emphasized); backed by the existing `daily_checkins.mood` column (NO migration — `mood` now = overall feeling). Energy/sleep demoted to optional depth.
**Check-in OFF the home.** Sofia: tracking is the core home action; the feeling card demoted it. Removed the inline day-view card (today + past). Feeling now lives in: (a) a daily **morning MOMENT** — `MorningCheckin.js`, once/day on first launch when today is unlogged, time-aware greeting at identity scale, always-dismissible, held confirmation, `localStorage checkin_moment_seen` guard, suppressed post-onboarding; (b) a persistent **"check in for today"** row in Insights › how you feel (opens `CheckinSheet`, now leading with the FeelingScale axis, not bars). Cleaned dead `FastRating`/`detailsOpen`.
**Insights polish.** Tab `interactions`→`guidance`; top-bar icon `Activity`→`TrendingUp` (Activity read as heart, worse now we show HRV); "worth moving" nudges dismissable (× → persists `dismissed_movers`; stays in the timing reference).
**APPLE HEALTH — activated (builds 19–20).** Path: Oura/Whoop sync sleep+HRV+resting-HR INTO Health → one HealthKit integration covers them; direct Oura/Whoop APIs = phase 2. `lib/health.js` rewritten for the **healthkit v14 (Nitro) API** (checked its type defs; old scaffold was a different API): `requestAuthorization({toShare,toRead})`, `{filter:{date:{startDate,endDate}}}`, `getMostRecentQuantitySample`. Reads sleep + HRV(SDNN) + resting HR. Installed `@kingstinct/react-native-healthkit@14.0.2` + `react-native-nitro-modules` (autolinks pod on build; dev client stays a guarded no-op — verified relaunch, no redbox). Added `com.apple.developer.healthkit` to `ios/Origin/Origin.entitlements` + `NSHealth{Share,Update}UsageDescription` to Info.plist. **Sofia enabled HealthKit on the App ID** (portal — required before build). Settings `apple health` CONNECT/CONNECTED (persists `health_enabled`; hidden until a Health build). **Readout** of last-night sleep/HRV/resting-HR in Insights › how you feel (null-safe, opt-in gated). **Build 19 connect flow verified on Sofia's iPhone**; **build 20** carries the readout + everything below. Phase 1b (sleep auto-fill, recovery track, report) after build 20 confirms real reads.
**User-supplement dictionary audit.** All 66 distinct names real users take → 31 recognized → **62**. `norm()` strips accents (Spanish class fixed at root); `hasKeyword` tolerates trailing plural-`s` (END-only; epa/tirzEPAtide guard re-verified). Ferrexel→iron (Levothyroxine+Ferrexel now flags); NeuroMag/Sensitol/Mitopure/etc.; 14 new entries w/ evidence-based timing (UC-II empty-stomach, isotretinoin with fat, allopurinol after food, licorice/methylene-blue AM, gabapentin/dutasteride/telmisartan); new rule levothyroxine↔fiber. Left 4 unidentified brands unrecognized (Amplifye/Nutricidin/Theracid/Urysia). TIMING-only throughout.
**Commits (`terminal-elevated`):** `6089974` `088c4d3` `4271a5c` `29fc804` `3873057` `b399363` `b892b99` `5375984` `4fa3227` `cba9dc0`. Builds 19 (submitted, connect-verified) + 20 (building).
...
**0ad. Build 20 + Apple Health verification (July 6 cont. 2) — Sofia on-device.** (1) Install **build 20** from TestFlight; **Settings → apple health → Connect** (already granted on build 19, persists); open **Insights › adherence › how you feel** and confirm the **"apple health · last night"** readout shows REAL sleep/HRV/resting-HR numbers (the on-device proof reads work; guarded no-op everywhere else). If numbers look wrong/absent, the v14 read mapping needs a tweak — ping me with what shows. (2) Then I wire **phase 1b**: sleep auto-fills the check-in, a **recovery** track (HRV/resting HR) in Trends, and both into the doctor report. (3) **Direct Oura/Whoop** OAuth (their Readiness/Recovery scores that don't flow into Health) = deferred phase 2 (needs Sofia's dev-app registrations + a token store). (4) **Dictionary follow-ups:** identify the 4 unrecognized user supps (**Amplifye, Nutricidin, Theracid, Urysia**); confirm **"Reta" = retatrutide** (tagged weekly GLP-1); confirm what **"Youth Peptides"** is (currently matches collagen via "peptides" — may be a false positive).

**0ac. Check-in + Insights redesign (July 6 cont. 2) — mostly self-contained, shipping in build 20.** The one-tap feeling scale, off-home + morning moment, Insights `guidance` rename, `TrendingUp` icon, and dismissable "worth moving" are code-complete and sim-verified. Open design calls Sofia flagged earlier but didn't decide: (a) morning-moment TIMING — fires on first launch of the day whenever that is (greeting adapts AM/PM) vs a hard morning-only gate; (b) whether the moment's `N days running` streak copy should stay or go quieter.
